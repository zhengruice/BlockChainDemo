...
/**
 * 如果值存在就 splice 删除元素，反之不处理
 * @param array 要操作的数据
 * @param value 要添加的值
 * @param key 可空，如果比较的是对象，可能存在地址不一样但值实际上是一样的情况，可以传此字段判断对象中唯一的字段，例如 id。不传则直接比较实际值
 * @returns {boolean} 成功 splice 返回 true，不处理返回 false
 */
export function spliceIfExist(array, value, key) {
  
  array.forEach(function(item, index, iterable){
	  if (key && (item[key] === value[key])) {
...
export function isEmpty(obj){
    //20210707 问题:由于0 与 ''的布尔值都是false，所以 0 == "" 为true
