...
export const OnlCgreportSelectModalMixin = {
  data(){
    return {
  ...
  methods:{
    ...
    classifySubTableData(res,subTableName){
      let { result } = res
      let dataSource = []
      let table =   this[`${subTableName}`].table
      //step1:获取表单配置
    ...
    // handleOnlCgreportSelectCallback(props, selectedRows,subTableName,callback){
    //   let orgFields = props.column.orgFields.split(',');
    //   let destFields = props.column.destFields.split(',');
    //   //step1:将报表中选择项与表单中已有项比较，删除没有项，添加新选择项
    //   this[`${subTableName}`].table.selectedRowKeys
    //     .splice(0, this[`${subTableName}`].table.selectedRowKeys.length);//清空数组
    //   this[`${subTableName}`].table.selectedRows
    //     .splice(0, this[`${subTableName}`].table.selectedRows.length);//清空数组
    //   //setp2:将表单中子表项全部清空，重新添加报表重选项
    //   this.eachAllTable((table, index) => {
    //     table.rows.forEach(function(item, idnex, array){
    //       table.removeRows(item.id);
    ...
    //     //子表单赋值
    //     for(let i = 0; i < selectedRows.length; i++) {
    //       this[`${subTableName}`].table.selectedRowKeys[i] = selectedRows[i][table.rowKey]; //记录子表已经选择项
    //       this[`${subTableName}`].table.selectedRows[i] = selectedRows[i]; //记录子表已经选择项
    //       let values = {};
    //       for(let j = 0; j < orgFields.length; j++) {
    //         if(!isEmpty(selectedRows[i][orgFields[j]])){
    //           values[destFields[j]] = selectedRows[i][orgFields[j]];
