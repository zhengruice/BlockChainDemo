    ...
    <j-form-container :disabled="formDisabled">
      ...
      <a-form-model ref="form" :model="model" :rules="validatorRules" slot="detail">
        <a-row>
          ...
          <a-col :span="12" >
            <a-form-model-item label="退回人" :labelCol="labelCol" :wrapperCol="wrapperCol" prop="employeeId">
...
<!--          </a-col>-->
    ...
    <a-tabs v-model="activeKey" @change="handleChangeTabs">
      <a-tab-pane tab="生产产品退回账单明细表" :key="refKeys[0]" :forceRender="true">

<!--        <a-button @click="handleBatchNoSelect(prodProductReturnBillDetailTable.props)" icon="plus" type="primary">选择</a-button>-->
...
<!--          :selectedRowKeys="prodProductReturnBillDetailTable.selectedRowKeys"-->
<!--          :selectedRows="prodProductReturnBillDetailTable.selectedRows"-->
<!--          @callback="(selectedRows)=>handleBatchNoSelectCallback(prodProductReturnBillDetailTable.props, selectedRows,'prodProductReturnBillDetailTable')"/>-->
        <a-button @click="handleOnlCgreportSelect(prodProductReturnBillDetailTable.table.props,'prodProductReturnBillDetailTable')" icon="plus" type="primary">选择</a-button>
        <onl-cgreport-select-modal
          ref="selectModal"
          title="选择产品"
          reportId="1408406865902465026"
          :width="1100"
          :params="prodProductReturnBillDetailTable.table.params"
          :multiSelectable="true"
          :selectedRowKeys="prodProductReturnBillDetailTable.table.selectedRowKeys"
          :selectedRows="prodProductReturnBillDetailTable.table.selectedRows"
          @callback="(selectedRows)=>handleOnlCgreportSelectCallback(prodProductReturnBillDetailTable.table.props, selectedRows,'prodProductReturnBillDetailTable')"/>
        <j-editable-table
          :ref="refKeys[0]"
          :loading="prodProductReturnBillDetailTable.loading"
          :columns="prodProductReturnBillDetailTable.columns"
          :dataSource="prodProductReturnBillDetailTable.dataSource"
          :maxHeight="300"
...
<script>
  ...
  export default {
    name: 'ProdProductReturnBillForm',
    ...
    data() {
      return {
        ...
        refKeys: ['prodProductReturnBillDetail', ],
        tableKeys:['prodProductReturnBillDetail', ],
        activeKey: 'prodProductReturnBillDetail',
        // 生产产品退回账单明细表
        prodProductReturnBillDetailTable: {
          loading: false,
          dataSource: [],
          table: {
            params: {},
            ...
            props: {//子表属性
              column: {
                orgFields: "id,product_id,product_code,product_measure_group_id,inventory_num,returnNum",
                destFields: "inventoryId,productId,productCode,productMeasureGroupId,inventoryNum,returnNum",
              },
            },
          },
        ...
        url: {
          add: "/prodProductReturnBill/prodProductReturnBill/add",
          edit: "/prodProductReturnBill/prodProductReturnBill/edit",
          queryById: "/prodProductReturnBill/prodProductReturnBill/queryById",
          prodProductReturnBillDetail: {
            list: '/prodProductReturnBill/prodProductReturnBill/queryProdProductReturnBillDetailByMainId'
          },
        }
      }
    },
    ...
    methods: {
      addBefore(){
        this.prodProductReturnBillDetailTable.dataSource=[]
      },
      addAfter (record) {
        this.visible = false;
        // 请求后台的填值规则接口地址
        const url = '/sys/fillRule/executeRuleByCode/'
        const ruleCode = 'prod_product_return_bill_on'
        putAction(url + ruleCode, {}).then(res => {
          ...
          if (res.success) {
            ...
            //this.edit(record);
            let that = this
            this.getPreRecords("/prodProductReturnBill/prodProductReturnBill/list",{},function(res){
      ...
      editAfter() {
        ...
        if (this.model.id) {
          ...
          let that = this
          this.requestSubTableData(this.url.prodProductReturnBillDetail.list, params, this.prodProductReturnBillDetailTable, function (res) {
            that.classifySubTableData(res,'prodProductReturnBillDetailTable')
          })
        }
      },
      //校验所有一对一子表表单
      ...
      classifyIntoFormData(allValues) {
        ...
        return {
          ...main, // 展开
          prodProductReturnBillDetailList: allValues.tablesValue[0].values,
        }
      },
      validateError(msg){
      ...
      warehouseIdChange(value){
        if(value)
          this.clearJEditTableByRef("prodProductReturnBillDetail")
      },
    }
  }
</script>
