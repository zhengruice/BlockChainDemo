<template>
  <a-spin :spinning="confirmLoading">
    <j-form-container :disabled="formDisabled">
      ...
      <a-form-model ref="form" :model="model" :rules="validatorRules" slot="detail">
        <a-row>
          ...
          <a-col :span="8" >
            <a-form-model-item label="收货日期" :labelCol="labelCol" :wrapperCol="wrapperCol" prop="receiveTime">
              <j-date placeholder="请选择收货日期" v-model="model.receiveTime" style="width: 100%" />
            </a-form-model-item>
          </a-col>
          <a-col :span="8" >
          ...
          <a-col :span="8" >
            <a-form-model-item label="收货仓库" :labelCol="labelCol" :wrapperCol="wrapperCol" prop="warehouseId">
              <j-tree-select
                ref="treeSelect"
                placeholder="请选择收货仓库"
    ...
    <a-tabs v-model="activeKey" @change="handleChangeTabs">
      <a-tab-pane tab="采购收货订单明细表" :key="refKeys[0]" :forceRender="true">
        <a-button @click="handleBatchNoSelect(purchaseMaterialReceiveBillDetailTable.props)" icon="plus" type="primary">选择采购订单</a-button>
        <main-sub-table-select-modal 
          ref="selectModal"
          title="选择采购订单"
          :tables="purchaseMaterialReceiveBillDetailTable.tables"
          @callback="handleBatchNoSelectCallback"/>
        
        <j-editable-table
          :ref="refKeys[0]"
          :loading="purchaseMaterialReceiveBillDetailTable.loading"
          :columns="purchaseMaterialReceiveBillDetailTable.columns"
          :dataSource="purchaseMaterialReceiveBillDetailTable.dataSource"
          :maxHeight="300"
...
<script>
  ...
  export default {
    name: 'PurchaseMaterialReceiveBillForm',
    ...
    data() {
      return {
        ...
        validatorRules: {
          ...
          warehouseId: [
             { required: true, message: '请选择收货仓库!'},
          ],
          employeeId: [
             { required: true, message: '请选择操作员!'},
          ],
        },
        refKeys: ['purchaseMaterialReceiveBillDetail', ],
        tableKeys:['purchaseMaterialReceiveBillDetail', ],
        activeKey: 'purchaseMaterialReceiveBillDetail',
        // 采购收货订单明细表
        purchaseMaterialReceiveBillDetailTable: {
          loading: false,
          dataSource: [],
          tables: {
            table1: {
            ...
            table2: {
              ...
              props: {//子表属性
                column: {
                    orgFields: "id,materialId,materialCode,materialUnit,materialModel,materialPrice,purchaseNum,purchaseMoney,nonReceiveNum,currentReceiveNum,currentReceiveMoney",
                    destFields: "superBillDetailId,materialId,materialCode,materialUnit,materialModel,materialPrice,purchaseNum,purchaseMoney,nonReceiveNum,currentReceiveNum,currentReceiveMoney",
                },
              },
              columns: [
                {
        ...
        url: {
          add: "/purchaseMaterialReceiveBill/purchaseMaterialReceiveBill/add",
          edit: "/purchaseMaterialReceiveBill/purchaseMaterialReceiveBill/edit",
          queryById: "/purchaseMaterialReceiveBill/purchaseMaterialReceiveBill/queryById",
          purchaseMaterialReceiveBillDetail: {
            list: '/purchaseMaterialReceiveBill/purchaseMaterialReceiveBill/queryPurchaseMaterialReceiveBillDetailByMainId'
          },
        }
      }
    },
    ...
    methods: {
      addBefore(){
        this.purchaseMaterialReceiveBillDetailTable.dataSource=[]
      },
      addAfter (record) {
        // 请求后台的填值规则接口地址
        const url = '/sys/fillRule/executeRuleByCode/'
        const ruleCode = 'purchase_receive_bill_no'
        ...
        let that = this
        //处理记录上一次的记录人
        this.getPreRecords("/purchaseMaterialReceiveBill/purchaseMaterialReceiveBill/list",{},function(res){
          if(res){
      ...
      editAfter() {
        ...
        if (this.model.id) {
          ...
          this.requestSubTableData(this.url.purchaseMaterialReceiveBillDetail.list, params, 
          this.purchaseMaterialReceiveBillDetailTable, this.classifySubTableData)
        }
      },
      //校验所有一对一子表表单
      ...
      classifyIntoFormData(allValues) {
        ...
        return {
          ...main, // 展开
          purchaseMaterialReceiveBillDetailList: allValues.tablesValue[0].values,
        }
      },
      validateError(msg){
      ...
      classifySubTableData(res){
        let { result } = res
        let dataSource = []
        let table1 = this.purchaseMaterialReceiveBillDetailTable.tables.table1
        let table2 = this.purchaseMaterialReceiveBillDetailTable.tables.table2
        //step1:获取表单配置
      ...
      onValueChange(event) {
        ...
        if (column.key === 'currentReceiveNum') {
          let currentReceiveNum = Number(value);
          let materialPrice = Number(row.materialPrice);
          let nonReceiveNum = Number(row.nonReceiveNum)
          if (!isNaN(currentReceiveNum) && !isNaN(nonReceiveNum)){
            if(currentReceiveNum > nonReceiveNum){
              this.validateError("本次收货数量不能大于未收货数量")
              return false;
            }
          }
          if (!isNaN(currentReceiveNum) && !isNaN(materialPrice)){
            target.setValues([{rowKey: row.id, values: {currentReceiveMoney:  currentReceiveNum * materialPrice}}]);
          }
          if (!isNaN(nonReceiveNum) && !isNaN(materialPrice)){
            target.setValues([{rowKey: row.id, values: {nonReceiveMoney:  nonReceiveNum * materialPrice}}]);
          }
