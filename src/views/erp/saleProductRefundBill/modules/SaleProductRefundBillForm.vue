  ...
  <a-spin :spinning="confirmLoading">
    <j-form-container :disabled="formDisabled">
      ...
      <a-form-model ref="form" :model="model" :rules="validatorRules" slot="detail">
        <a-row>
          <a-col :span="12" >
            <a-form-model-item label="退款单号" :labelCol="labelCol" :wrapperCol="wrapperCol" prop="billNo">
              <a-input v-model="model.billNo" placeholder="请输入退款单号" disabled></a-input>
            </a-form-model-item>
          </a-col>
          <a-col :span="12" >
          ...
          <a-col :span="12" >
            <a-form-model-item label="退款日期" :labelCol="labelCol" :wrapperCol="wrapperCol" prop="refundTime">
              <j-date placeholder="请选择退款日期" v-model="model.refundTime" style="width: 100%" />
            </a-form-model-item>
          </a-col>
          <a-col :span="24">
    ...
    <a-tabs v-model="activeKey" @change="handleChangeTabs">
      <a-tab-pane tab="产品销售退款订单明细表" :key="refKeys[0]" :forceRender="true">
        <a-button @click="handleBatchNoSelect(saleProductRefundBillDetailTable.props)" :disabled="formDisabled" icon="search" type="primary">选择</a-button>
        <sub-table-select-modal
          ref="selectModal"
          :fullscreen="true"
          :tables="saleProductRefundBillDetailTable.tables"
          @callback="handleBatchNoSelectCallback"/>
        <j-editable-table
          :ref="refKeys[0]"
          :loading="saleProductRefundBillDetailTable.loading"
          :columns="saleProductRefundBillDetailTable.columns"
          :dataSource="saleProductRefundBillDetailTable.dataSource"
          :maxHeight="300"
...
<script>
  ...
  export default {
    name: 'SaleProductRefundBillForm',
    ...
    data() {
      return {
        ...
        validatorRules: {
           ...
           refundTime: [
              { required: true, message: '请输入退款日期!'},
           ],
        },
        refKeys: ['saleProductRefundBillDetail', ],
        tableKeys:['saleProductRefundBillDetail', ],
        activeKey: 'saleProductRefundBillDetail',
        // 产品销售退款订单明细表
        saleProductRefundBillDetailTable: {
          loading: false,
          dataSource: [],
          tables: {
            table1: {
        ...
        url: {
          add: "/saleProductRefundBill/saleProductRefundBill/add",
          edit: "/saleProductRefundBill/saleProductRefundBill/edit",
          queryById: "/saleProductRefundBill/saleProductRefundBill/queryById",
          saleProductRefundBillDetail: {
            list: '/saleProductRefundBill/saleProductRefundBill/querySaleProductRefundBillDetailByMainId'
          },
        }
      }
    },
    ...
    methods: {
      addBefore(){
        this.saleProductRefundBillDetailTable.dataSource=[]
      },
      addAfter (record) {
        this.visible = false;
        // 请求后台的填值规则接口地址
        const url = '/sys/fillRule/executeRuleByCode/'
        const ruleCode = 'sale_refund_bill_no'
        putAction(url + ruleCode, {}).then(res => {
          ...
          if (res.success) {
            ...
            let that = this
            //处理记录上一次的记录人
            this.getPreRecords("/saleProductRefundBill/saleProductRefundBill/list",{},function(res){
      ...
      editAfter() {
        ...
        if (this.model.id) {
          ...
          this.requestSubTableData(this.url.saleProductRefundBillDetail.list, params, this.saleProductRefundBillDetailTable)
        }
      },
      //校验所有一对一子表表单
      ...
      classifyIntoFormData(allValues) {
        ...
        return {
          ...main, // 展开
          saleProductRefundBillDetailList: allValues.tablesValue[0].values,
        }
      },
      validateError(msg){
      ...
      handleBatchNoSelectCallback(selectedRowKeys1,selectedRows1,selectedRowKeys2,selectedRows2){
        ...
        let table1 = this.saleProductRefundBillDetailTable.tables.table1
        let table2 = this.saleProductRefundBillDetailTable.tables.table2
        //step1:获取表单配置
