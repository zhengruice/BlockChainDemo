<template>
  <a-spin :spinning="confirmLoading">
          ...
          <!-- <a-col :span="12" >
            <a-form-model-item label="材料类别" :labelCol="labelCol" :wrapperCol="wrapperCol" prop="materialCategoryId">
              <j-tree-select
                ref="treeSelect"
                placeholder="请选择材料类别"
                v-model="model.materialCategoryId"
    ...
    <a-tabs v-model="activeKey" @change="handleChangeTabs">
      <a-tab-pane tab="材料库存盘点明细" :key="refKeys[0]" :forceRender="true">
        <a-button @click="handleBatchNoSelect(stockMaterialCheckBillDetailTable.table.props)" icon="plus" type="primary">选择</a-button>
        <onl-cgreport-select-modal
          ...
          :selectedRowKeys="stockMaterialCheckBillDetailTable.table.selectedRowKeys"
          :selectedRows="stockMaterialCheckBillDetailTable.table.selectedRows"
          @callback="(selectedRows)=>handleBatchNoSelectCallback(stockMaterialCheckBillDetailTable.table.props, selectedRows)"/>

        <j-editable-table
          :ref="refKeys[0]"
          :loading="stockMaterialCheckBillDetailTable.loading"
          :columns="stockMaterialCheckBillDetailTable.columns"
          :dataSource="stockMaterialCheckBillDetailTable.dataSource"
          :maxHeight="300"
          ...
          <!-- <template v-slot:batchNoSelectModal="props" style="padding-left: 0">
            ...
            <onl-cgreport-select-modal
              ...
              :width="1100"
              :multiSelectable="true"
              :selectedRowKeys="stockMaterialCheckBillDetailTable.selectedRowKeys"
              :selectedRows="stockMaterialCheckBillDetailTable.selectedRows"
...
<script>
  ...
  export default {
    name: 'StockMaterialCheckBillForm',
    ...
    data() {
      return {
        ...
        refKeys: ['stockMaterialCheckBillDetail', ],
        tableKeys:['stockMaterialCheckBillDetail', ],
        activeKey: 'stockMaterialCheckBillDetail',
        // 材料库存盘点明细
        stockMaterialCheckBillDetailTable: {
          loading: false,
          dataSource: [],
          table: {
            params: {},
            ...
            props: {//子表属性
              column: {
                  orgFields: "id,material_id,material_category_id,material_code,supplier_id,inventory_num,check_num,profit_and_loss",
                  destFields: "materialInventoryId,materialId,materialCategoryId,materialCode,supplierId,inventoryNum,checkNum,profitAndLoss",
              },
            },
          },
          ...
          columns: [
            ...
            /* {
              ...
              orgFields: "material_id,material_category_id,material_code,inventory_num",
              destFields: "materialId,materialCategoryId,materialCode,inventoryNum",
              width:"45px",
            }, */
            /* {
              title: '选择',
              ...
              popupCode: 'report_material_inventory',
              field: 'name',
              orgFields: "material_id,material_category_id,material_code,inventory_num",
              destFields: "materialId,materialCategoryId,materialCode,inventoryNum",
              width:"45px",
        ...
        url: {
          add: "/stockMaterialCheckBill/stockMaterialCheckBill/add",
          edit: "/stockMaterialCheckBill/stockMaterialCheckBill/edit",
          queryById: "/stockMaterialCheckBill/stockMaterialCheckBill/queryById",
          stockMaterialCheckBillDetail: {
            list: '/stockMaterialCheckBill/stockMaterialCheckBill/queryStockMaterialCheckBillDetailByMainId'
          },
        }
      }
    },
    ...
    methods: {
      addBefore(){
        this.stockMaterialCheckBillDetailTable.dataSource=[]
      },
      addAfter (record) {
        // 请求后台的填值规则接口地址
        const url = '/sys/fillRule/executeRuleByCode/'
        const ruleCode = 'material_check_bill_no'
      ...
      editAfter() {
        ...
        if (this.model.id) {
          ...
          this.requestSubTableData(this.url.stockMaterialCheckBillDetail.list, params,
            this.stockMaterialCheckBillDetailTable, this.classifySubTableData)
        }
      },
      //校验所有一对一子表表单
      ...
      classifyIntoFormData(allValues) {
        ...
        return {
          ...main, // 展开
          stockMaterialCheckBillDetailList: allValues.tablesValue[0].values,
        }
      },
      validateError(msg){
      ...
      classifySubTableData(res){
        let { result } = res
        let dataSource = []
        let table = this.stockMaterialCheckBillDetailTable.table
        //step1:获取表单配置
      ...
      handleBatchNoSelect(props){
        ...
        this.$refs.form.validate((pass,obj)=>{
          if(pass){
            ...
            queryParams['material_category_id'] = this.model.materialCategoryId;
      ...
      handleBatchNoSelectCallback(props, selectedRows){
        ...
        this.stockMaterialCheckBillDetailTable.table.selectedRowKeys
        .splice(0,this.stockMaterialCheckBillDetailTable.table.selectedRowKeys.length);//清空数组
        this.stockMaterialCheckBillDetailTable.table.selectedRows
        .splice(0,this.stockMaterialCheckBillDetailTable.table.selectedRows.length);//清空数组
        ...
        this.eachAllTable((table, index) => {
          ...
          for(let i = 0; i < selectedRows.length; i++) {
            this.stockMaterialCheckBillDetailTable.table.selectedRowKeys[i] = selectedRows[i][rowKey]; //记录子表已经选择项
            this.stockMaterialCheckBillDetailTable.table.selectedRows[i] = selectedRows[i]; //记录子表已经选择项
            let values = {};
